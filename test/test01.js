...
describe('ЗАДАЧА 1: функция numDots', function(){
	...
	it('дает 5 для строки из пяти точек', function() {
		const numDots = require('../L05-01.js');
		assert.strictEqual(numDots('.....'), 5, "неверно для '.....'");
	});

	it('дает строки без точек', function() {
		const numDots = require('../L05-01.js');
		assert.strictEqual(numDots('window'), 0, "неверно для 'window'");
	}); 
