...
describe('ЗАДАЧА 1: функция numDots', function(){
    ...
    //добавьте тест для длинной строки без точек
    it('дает 0 для строки без точек', function(){
        const numDots = require('../L05-01.js');
        assert.strictEqual(numDots("marsenmaksimov"), 0, "неверно для строки marsenmaksimov");
    });

    //добавьте тест для строки, целиком состоящей из точек
    it('дает прав ответ для строки из одних точек', function(){
        const numDots = require('../L05-01.js');
        assert.strictEqual(numDots("...."), 4, "неверно для строки ....");
    });
