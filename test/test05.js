...
describe('ЗАДАЧА 5: функция isDelResult', function(){
	...
	it('дает истину для пары ALABAMA, ALMA', function(){
		...
		assert(isDelResult("ALABAMA", "ALMA") == true, "неверно для 'ALABAMA', 'ALMA'");
	});

	it('дает истину для пары PANCAKE, CAKKE', function(){
		const isDelResult = require('../L05-05.js');
		assert(isDelResult("PANCAKE", "CAKKE") == false, "неверно для 'PANCAKE', 'CAKKE'");
	});
